...
Record mappings are stored in `data/record_mappings.json` (created automatically).

---

## API overview

- **Motors:** `GET/POST /api/motors/tone-arm`, `GET /api/motors/turntable`, `POST /api/motors/turntable/start`, `POST /api/motors/turntable/stop`
...
- **Spotify auth:** `GET /api/spotify/auth-url`, `GET /api/spotify/callback`, `POST /api/spotify/complete-login` (manual: send `redirect_url` or `code`)
